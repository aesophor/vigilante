...
#define VGLOG(severity, format, ...)                                                          \
  cocos2d::log("[%s] [%s: %d] " format, (vigilante::logger::_kSeverityStr[severity].c_str()), \
...
enum Severity { ERROR, WARNING, INFO, SIZE };
const std::array<std::string, Severity::SIZE> _kSeverityStr = {{"ERROR", "WARNING", "INFO"}};
